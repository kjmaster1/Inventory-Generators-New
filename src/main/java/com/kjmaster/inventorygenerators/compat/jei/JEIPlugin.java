...
public class JEIPlugin implements IModPlugin {
    ...
    public void registerRecipeCatalysts(IRecipeCatalystRegistration registration) {
        ...
        Item culinaryGenerator = InventoryGeneratorModule.INVENTORY_CULINARY_GENERATOR.get();
        items.add(culinaryGenerator);
        Item deathGenerator = InventoryGeneratorModule.INVENTORY_DEATH_GENERATOR.get();
        items.add(deathGenerator);
        Item endGenerator = InventoryGeneratorModule.INVENTORY_END_GENERATOR.get();
        items.add(endGenerator);
        Item explosiveGenerator = InventoryGeneratorModule.INVENTORY_EXPLOSIVE_GENERATOR.get();
        items.add(explosiveGenerator);
        Item frostyGenerator = InventoryGeneratorModule.INVENTORY_FROSTY_GENERATOR.get();
        items.add(frostyGenerator);
        Item furnaceGenerator = InventoryGeneratorModule.INVENTORY_FURNACE_GENERATOR.get();
        items.add(furnaceGenerator);
        Item halitosisGenerator = InventoryGeneratorModule.INVENTORY_HALITOSIS_GENERATOR.get();
        items.add(halitosisGenerator);
        Item netherStarGenerator = InventoryGeneratorModule.INVENTORY_NETHER_STAR_GENERATOR.get();
        items.add(netherStarGenerator);
        Item overclockedGenerator = InventoryGeneratorModule.INVENTORY_OVERCLOCKED_GENERATOR.get();
        items.add(overclockedGenerator);
        Item pinkGenerator = InventoryGeneratorModule.INVENTORY_PINK_GENERATOR.get();
        items.add(pinkGenerator);
        Item potionGenerator = InventoryGeneratorModule.INVENTORY_POTION_GENERATOR.get();
        items.add(potionGenerator);
        Item slimeyGenerator = InventoryGeneratorModule.INVENTORY_SLIMEY_GENERATOR.get();
        items.add(slimeyGenerator);
        Item survivalistGenerator = InventoryGeneratorModule.INVENTORY_SURVIVALIST_GENERATOR.get();
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        vanillaFuelRecipes.forEach(fuelRecipe -> {
            ...
            fuelRecipes.add(new GeneratorRecipe(
                    ResourceLocation.fromNamespaceAndPath(InventoryGenerators.MODID, "survivalist_generator"),
                    List.of(Ingredient.of(fuelRecipe.getInputs().stream())), Ingredient.of(survivalistGenerator), burnTime, survivalistGeneratorRfPerTick));
            burnTime = (int) Math.ceil(fuelRecipe.getBurnTime() / furnaceGeneratorDivisor);
            fuelRecipes.add(new GeneratorRecipe(
                    ResourceLocation.fromNamespaceAndPath(InventoryGenerators.MODID, "furnace_generator"),
                    List.of(Ingredient.of(fuelRecipe.getInputs().stream())), Ingredient.of(furnaceGenerator), burnTime, furnaceGeneratorRfPerTick));
            burnTime = (int) Math.ceil(fuelRecipe.getBurnTime() / overclockedGeneratorDivisor) + 1;
            fuelRecipes.add(new GeneratorRecipe(
                    ResourceLocation.fromNamespaceAndPath(InventoryGenerators.MODID, "overclocked_generator"),
                    List.of(Ingredient.of(fuelRecipe.getInputs().stream())), Ingredient.of(overclockedGenerator), burnTime, overclockedGeneratorMinRfPerTick));
        });
        ...
        foodList.forEach(stack -> {
            ...
            if (properties != null) {
                ...
                foodRecipes.add(new GeneratorRecipe(
                        ResourceLocation.fromNamespaceAndPath(InventoryGenerators.MODID, "culinary_generator"),
                        List.of(Ingredient.of(stack)), Ingredient.of(culinaryGenerator), burnTime, energyRate));
            }
        });
